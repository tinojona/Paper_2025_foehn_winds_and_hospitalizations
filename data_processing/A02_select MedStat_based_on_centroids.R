...
# - load the station locations and create a buffer around them
# - determine which of the centroids fall inside the buffer
# - MANUALLY assign through plotting which centroids are assigned to which station buffer
#   and save the Medstat regions to every station in the Medstat_region_per_buffer_size folder
#   for every buffer size. (3000-15000)
...
mesh$geometry <- st_transform(mesh$geometry, crs = crs(raster_data))
station_loc$geometry <- st_transform(station_loc$geometry, crs = crs(raster_data))
...
# uniform crs code end

#----


### Elevation Map Plot of centroid selected Medstats 8km
#----
...
### Sensitivity analysis: Elevation Map Plot of centroid selected Medstats 5km+10km
#----

# Note: the plot is configured for a buffer radius of 10km
# therefore the for loop above has to be run for 10km and the preparation code for plotting
...
# CAREFUL!!
# Note: the plot is configured for a buffer radius of 5km
# therefore the for loop above has to be run for 5km and the preparation code for plotting
...
### Notes about the total number of Medstats per buffer radius
#----
...
### Dummyplots for testing Medstats
#----
