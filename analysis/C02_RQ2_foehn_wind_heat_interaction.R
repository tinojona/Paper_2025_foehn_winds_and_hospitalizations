################################################################################
### RQ2: the foehn wind temperature interaction
################################################################################
...
### Figure 3: Temperature hospitalization association (Model 3) and distinguishing
### between the association on foehn wind days and non foehn wind days (Model 4)
### #----
...
pdf("output/figures/Figure3_only_allcause_bluered.pdf", width = 8.5, height =6.5)
...
### Figure 4 - foehn winds modification of temperature for cold and heat
#----

# (a) Cumulative relative risk (Model 4) for subpopulations for cold temperatures
# (-8.9°C, 1st percentile) with 95% confidence intervals (minimum hospitalizations
# temperatures and their percentiles: all [16°C, 78th], male [16°C, 78th], female [15°C, 74th],
# 64 years and younger [14°C, 70th], older than 64 years [16°C, 78th], circulatory [11°C,                                                                                                                                 57th], respiratory [19°C, 90th], infectious [20°C, 91st], genitourinary [0°C, 14th], mental
# [15°C, 74th]). (b) Cumulative relative risk (Model 4) for subpopulations for heat (24.7°C,
# 99th percentile) with 95% confidence intervals and the same minimum hospitalization
# temperatures.
...
pdf("output/figures/Figure4.pdf", width = 6, height = 4.5)
...
legend(x = 0.2, y = 1 , ncol = 1,
       ...
       col = c("steelblue", "steelblue"), bty = "n", lwd=c(2,2), lty = c(2,1), cex = c(.9,.9),
       y.intersp = 1.5,
       x.intersp = 1,
...
legend(x = -0.04, y = 1 , ncol = 1,
       ...
       col = c("brown2", "brown2"), bty = "n", lwd=c(2,2), lty = c(2,1), cex = c(.9,.9),
       y.intersp = 1.5,
       x.intersp = 1,
       seg.len = 3
       # text.width = c(0.45, 0.55)
...
for(i in 1:length(table_estimates_cold$pred)) {
  ...
  lty <- ifelse(table_estimates_cold$model[i] == "temp + foehn", 2, 1)
  col <- "steelblue"
...
### sFigure 5: Interaction term for Model 4 for all-cause hospitalizations
#----

# Cumulative relative risk of the interaction between foehn winds and
# temperature with 95% confidence interval for all-cause hospitalizations. The relative risk
# and the confidence interval shown inside the figure correspond to the relative risk at the
# exposure of -8.9 °C (1st percentile)(bottom) and 24.7°C (99th percentile)(top).
...
### sFigure 6: Temperature hospitalization association in the presence and absence
### of foehn winds for sobpopulations
#----

# Cumulative relative risk with 95% confidence intervals and minimum
# hospitalization temperatures with their percentile for (a) male [16°C, 78th], (b) female
# [15°C, 74th], (c) 64 years and younger [14°C, 70th], (d) older than 64 years [16°C, 78th],
# (e) circulatory [11°C, 57th], (f) respiratory [19°C, 90th], (g) infectious [20°C, 91st], (h)
# genitourinary [0°C, 14th], (i) mental hospitalization [15°C, 74th] with a binary foehn wind
...
### sFigure 7: Interaction term for Model 4 for subpopulations
#----

# Cumulative relative risk of the interaction between foehn wind and
# temperature with 95% confidence interval for (a) male, (b) female, (c) 64 years and
# younger, (d) older than 64, (e) circulatory, (f) respiratory, (g) infectious, (h) genitourinary,
# (i) mental hospitalizations. The relative risk and the confidence interval shown inside
# each figure correspond to the relative risk at the exposure of -8.9 °C (1st
# percentile)(bottom) and 24.7°C (99th percentile)(top).
...
### sTable 7: Sensitivity analysis on the time period (starting in 2008) and the
### foehn wind aggregation method (only full foehn wind) on the heat interaction
#----
...
# (only full foehn wind) on the interaction between heat (24.7°C, 99th percentile) and foehn
# winds (Model 4) with fixed minimum hospitalization temperatures for every subpopulation.
# The 2 left columns represent the baseline risk shown in Figure 4b.
