...
library(dlnm);library(splines);library(ggplot2);library(viridis);library(gnm); library(dplyr);library(knitr);library(kableExtra); library(magick)

rm(list=ls())
buffer = 8000 # buffer size

data = read.csv(paste0("../../data/MedStat_aggregated/centroid_aggregated/hosp_buffer_", buffer, ".csv")) # data
...
colnames(table_estimates_transposed) <- c("relative risk foehn [CI]", "relative risk foehn + temperature [CI]")
...
# write.csv(table_estimates_transposed, file = "C:/Users/tinos/Documents/Master - Climate Science/3 - Master Thesis/plots/paper/rq1_table.csv")
```


PLOT of exposure response for all ~ foehn wind for all cause hospitalizations

```{r}
# model
# mod_nm1 <- gnm(all ~ cb.foehn, data = data,  family=quasipoisson(), eliminate=stratum_dow, subset=ind_dow>0)

# prediction
# pred_nm1 <- crosspred(cb.foehn, mod_nm1, at=0:288, cumul=FALSE, cen = 0)

# png("C:/Users/tinos/Documents/Master - Climate Science/3 - Master Thesis/Defense/rq1_foehn_only_biglab.png", width = 1800, height =1800, res = 300)


# png("C:/Users/tinos/Desktop/test.png", width = 1800, height = 1800, res = 300)
...
plot(pred_nm1,
     "overall",
     xlab = "foehn wind intensity",
     ylab = "relative risk",
     col = "gold2",
     ci.arg = list(col = alpha(colour = "gold3", .25)),
     lwd = 3,
     cex.lab = 2,
     cex.axis = 2
     )
...
Table sensitivity analysis of buffer size!

```{r sensitivity}
sensitivity_table = data.frame(
  bs_5= rep(NA,2),
  bs_10=rep(NA,2))
colnames(sensitivity_table) = c("Model1", "Model2")
...
#### TABLE Sensitivity regarding the foehn wind aggregation method (only full foehn wind)
...
# index to include only stratum that have hosp counts
ind_dow_ff = tapply(data_ff$all, data_ff$stratum_dow, sum); ind_ff = tapply(data_ff$all, data_ff$stratum, sum)
...
write.csv(table_estimates_transposed, "pelim.csv")
...
3D PLOT OF FOEHN Wind


```{r}

mod <- gnm(all ~ cb.foehn, data = data,  family=quasipoisson(), eliminate=stratum_dow, subset=ind_dow>0)
prednew <- crosspred(cb.foehn, mod, cumul=FALSE, cen = 20)
...
*Table 1:Sensitivity analysis of the lag period *
...
for (i in 1:length(groups_id)) {
  ...
  # prediction
  pred_nm1 <- crosspred(cb.foehn, mod_nm1, at=0:288, cumul=TRUE, cen = 0)
