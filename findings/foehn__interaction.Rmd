...
for (i in 1:length(groups_id[1])) {
  ...
  abline(v =quantile(data$temp, .99), col = "black", lty = 2)
    ...
    legend("top", ncol = 1, legend = c("temp with foehn", "temp without foehn"), col = c("green4", "gold2"),
         bty = "n", lwd=c(2,2), cex = 0.7)
  
  # text(25, 3 , labels = "(a)", pos = 2, cex = 0.7)
  }
...
plot(pred_modif_new_sum,              ## cumulative exposure
  ...
  legend("topleft", ncol = 1, legend = c("temp with foehn", "temp without foehn"), col = c("green4", "gold2"),
         bty = "n", lwd=c(2,2), cex = 0.7)
...
table_estimates$categories[7:8] = "age <= 64"
table_estimates$categories[9:10] = "age > 64"
...
ggplot(table_estimates, aes(y = categories, x = pred, color = model)) +
  ...
  scale_color_manual(
    ...
    labels = c("temp + foehn" = "temp with foehn", "temp - foehn" = "temp without foehn"),  # Map the levels to custom labels
    name = "Model Type:"  # Custom legend title
  ) +
  theme(
...
```{r results_subgroups_interaction, fig.width=9, fig.height=9, fig.align='center'}
# png("C:/Users/tinos/Documents/Master - Climate Science/3 - Master Thesis/plots/paper/rq3_subgroups_plot_interaction_term.png", width = 1800, height =1800, res = 300)
...
for (i in 1:length(groups_id[2:10])) {
  ...
  # prediction with and without foehn
  pred_modif   <- crosspred(modif, mod_modif, cen = 0, at = c(0,1), cumul=FALSE)
  pred_modif_rev <- crosspred(modif_rev, mod_modif_rev, cen = 0, at = c(0,1), cumul=FALSE)
  ...
  text("topleft", labels = paste0("(", letters[i], ")"), pos = 2)}
# max(pred_modif_new$allRRhigh)
...
*Figure 1: Subgroups interaction term*
